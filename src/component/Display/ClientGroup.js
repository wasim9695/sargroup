...
import { getCountryList, getStateList, getCityList, saveCLientGroup,
    updateClientGroup, getClientGroupList, getShowClientGroupList } from "../../service/AllApiData-service";
    const  ClientGroup = () => {
    const [getcountry, setGetCountry] = useState([]);
    const [getState, setGetState] = useState([]);
    const [getCity, setGetCity] = useState([]);
    const [getClientLists, setClientLists] = useState([]);
    const [getClientGoupLists, setClientGoupLists] = useState([]);
    const [cg_name, setCg_name] = useState("");
    const [vtc_vendor_code, setVtc_vendor_code] = useState("");
   ...
   const getAllCountry = async () => {
    ...
    const getAllCity = async () => {
      ...
      const getAllClientsGroupList = async () => {
        // const data = { page:page, limit:'25', search:'' };
         await getClientGroupList().then(res => {
        // console.log("ddd", res.data);
        setClientGoupLists(res.data.data);
          })
          .catch(err => {
            console.log(err);
          })
   ...
   const displayedItems = getClientGoupLists.slice(startIndex, startIndex + itemsPerPage);


      const save = async () => {
        const data = {cg_name,vtc_vendor_code,address_line1,address_line2,
         ...
         await saveCLientGroup(data).then(res => {
            console.log(res);
      
            alert("Save Successfully");
            getAllClientsGroupList();
            setHideTabs(true);
        ...
        <div>
            ...
            <div className="col-md-12">
                <div className="row" hidden={hideTabs}>
                    ...
                    <h3 className="font-weight-bolder text-info text-gradient text-center display-7">Client Group</h3>
                    <div className="row">
...
<label>VTC Vendor Code:</label>
<div className="mb-3">
<input type="text" onChange={e => setVtc_vendor_code(e.target.value)} name="vtc_vendor_code" 
value={vtc_vendor_code} className="form-control" placeholder="VTC Vendor Code" aria-label="VTC Vendor Code"/>
</div>
<label>Client Group Name:</label>
<div className="mb-3">
<input type="text" onChange={e => setCg_name(e.target.value)} name="cg_name" 
value={cg_name} className="form-control" placeholder="Client Group Name" aria-label="Client Group Name"/>
</div>
                                ...
                                <div className='row mt-2'>
                                    <nav aria-label="Page navigation example">
                                        <ul className="pagination"
                                            ...
                                            <li className="page-item">
                                                <a onClick={handleNextClick} disabled={page === Math.ceil(getAllClientsGroupList.length / itemsPerPage)}  className={"page-link"+" "+ (page === Math.ceil(getAllClientsGroupList.length / itemsPerPage) ? "badge-primarys " : "badge-dangers ")} href="javascript:;" aria-label="Next">
